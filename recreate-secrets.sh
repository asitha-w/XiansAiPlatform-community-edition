...
for service in keycloak postgresql temporal server; do
    if [ "$service" = "server" ]; then
        example_file="${service}/.env.local.example"
...
update_env_file "server/.env.local" "Certificates__AppServerPfxBase64" "$EXISTING_CERT"
update_env_file "server/.env.local" "WebSockets__Secrets__99x.io" "$WEBSOCKET_99X_SECRET"
update_env_file "server/.env.local" "WebSockets__Secrets__default" "$WEBSOCKET_DEFAULT_SECRET"
...
echo "   📜 Server certificate: Using existing certificate"
echo ""
