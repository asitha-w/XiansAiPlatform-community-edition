...
DESCRIPTION:
    ...
    Now configured for prod.xiansai.net domain with wildcard DNS support.
...
CONFIGURATION:
    Domain: prod.xiansai.net (configured via Azure DNS)
    Subdomains: api.prod.xiansai.net, auth.prod.xiansai.net, temporal.prod.xiansai.net
...
log "🚀 Starting XiansAi Production HTTPS Setup for prod.xiansai.net..."

# Check if .env file exists
if [ ! -f ".env" ]; then
    error ".env file not found in nginx directory!"
    error "The .env file should already be configured for prod.xiansai.net"
    exit 1
fi
...
log "✅ XiansAi Production HTTPS setup completed successfully for prod.xiansai.net!"